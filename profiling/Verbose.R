...
for (progeny.index in 56:56){
  ...
  for(chromosome in chromosomes) #by chromosome
  {
    ...
    vit<-ViterbiWithMissingData(burnin.obs,
                                burnin.start.p, 
                                trans.p,
    ...
    vit<-ViterbiWithMissingData(obs,start.p, trans.p,mom.chromatid,dad.chromatid,0.1)
    vitrowmax.2 <- apply(vit, 1, function(x) which.max(x))
    ...
    cat(mom.chromatid$maternal[1:20],"\n",mom.chromatid$paternal[1:20],"\n",
        dad.chromatid$maternal[1:20],"\n",dad.chromatid$paternal[1:20],"\n")
    cat("\n",obs[1:20],"\n")
    cat("\n",estimate[1:20],"\n")
    ...
    if(CountRecombEvents(estimated)>0){
      ...
      for(event.index in 1:length(eventIndex)){
        ...
        cat(progeny.index,
            ...
            sep="\t","\n",file="hmm-summary-co.txt",append=TRUE)
