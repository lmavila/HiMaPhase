...
#parent.col.names<-names(parents)
...
chunkNames<-names(parents)
#chunkNames[grep("chunk",chunkNames)]
for(chunk.name in chunkNames[grep("chunk",chunkNames)]){
  cat(chunk.name,nlevels(as.factor(paste(parents$chr,parents[,chunk.name]))),"\n")    
}
